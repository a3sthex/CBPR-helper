...
function cyberdeckSheetHtml(characterData,derived,mine){const decks=(characterData.inventory||[]).filter(item=>item.cat==='net_stuff'&&item.mechanics?.type==='Cyberdeck');if(!decks.length)return `<div class="empty small">${T('No Cyberdeck instances in this Dossier.','В Dossier нет Cyberdeck instances.')}</div>`;return decks.map(deck=>{const effective=derived.effective_cyberdecks?.[deck.instance_id]||{},pools=effective.pools||{};return `<article class="panel mb cyberdeck-card"><div class="row" style="justify-content:space-between"><div><h3>${esc(deck.custom_name||deck.name)}</h3><div class="mechanic-chips">${Object.entries(pools).map(([name,pool])=>`<span class="chip">${esc(name)} ${pool.used}/${pool.total}</span>`).join('')}<span class="chip">Total ${effective.slots_used||0}/${effective.slots_total||0}</span></div></div>${mine?`<button class="btn-sm" data-manage-cyberdeck="${deck.instance_id}">${T('Manage Loadout','Управление загрузкой')}</button>`:''}</div><div class="grid cols-2 mt"><div><b>Hardware</b>${(effective.hardware||[]).map(item=>`<div class="small">▣ ${esc(item.name)} · ${item.slots} slots</div>`).join('')||'<div class="small muted">—</div>'}</div><div><b>Programs</b>${(effective.programs||[]).map(item=>`<div class="small">◆ ${esc(item.name)} · ${item.slots} slots · ${esc(item.program_class||'')}</div>`).join('')||'<div class="small muted">—</div>'}</div></div><div class="small warn-text mt">${T('PROGRAM RUNTIME IS MANUAL · Rez/Derez and REZ state arrive in B.8.2','PROGRAM RUNTIME ВРУЧНУЮ · Rez/Derez и состояние REZ появятся в B.8.2')}</div></article>`;}).join('');}
