...
function cyberwareLifecycleHtml(items,loadout,mine) {
  ...
  return `${payloads}${issues?`<div class="cyberware-issues mb"><b>${T('Host binding requires attention','Требуется исправить привязку host')}</b>${issues}</div>`:''}${hosts?`<div class="cyber-host-grid">${hosts}</div>`:''}${cyberweapons}${standalone?`<h3 class="mt">${T('Installed Standalone Cyberware','Установленная самостоятельная Cyberware')}</h3>${standalone}`:''}${staged?`<h3 class="mt">${T('Staged Cyberware','Подготовленная Cyberware')}</h3>${staged}`:''}`;
}
