...
async function route() {
  ...
  // 20.4: карта живёт внутри раздела City — отдельного пункта навигации у неё нет.
  const navRoute = activeRoute === 'map' ? '' : activeRoute;
  $$('[data-route]').forEach(anchor => {
    const active = anchor.dataset.route === navRoute;
    anchor.classList.toggle('active', active);
    if (active) anchor.setAttribute('aria-current', 'page');
    else anchor.removeAttribute('aria-current');
  });
  document.body.dataset.workspace = activeRoute === 'gm' ? 'gm' : 'network';
  const moreButton=$('#mobile-more-toggle');if(moreButton)moreButton.classList.toggle('active',['database','market','quick-reference','crew','stash','chronicle','memorial','personas','guides','profile','gm','admin'].includes(navRoute));
