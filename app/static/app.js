...
async function viewSheet(id) {
  ...
  <nav class="sheet-tabs"><button data-sheet-jump="sheet-overview">Overview</button><button data-sheet-jump="sheet-skills">Skills</button><button data-sheet-jump="sheet-combat">Combat</button><button data-sheet-jump="sheet-gear">Gear</button><button data-sheet-jump="sheet-garage">Garage</button><button data-sheet-jump="sheet-cyberware">Cyberware</button><button data-sheet-jump="sheet-lifepath">Lifepath</button></nav>
  ...
  <div class="grid cols-2 sheet-layout" style="gap:18px">
    <div>
      ...
      <div class="panel mb" id="sheet-garage">
        ...
        ${vehicles.length?vehicles.map(vehicle=>{const mechanics=vehicle.mechanics||{},installed=(d.modifications||[]).filter(mod=>mod.host_instance_id===vehicle.instance_id);return `<article class="panel mb vehicle-garage-card"><div class="row" style="justify-content:space-between"><div><h3>${esc(vehicle.custom_name||vehicle.name)}</h3><div class="mechanic-chips"><span class="chip">SDP ${esc(mechanics.sdp??'—')}</span>${mechanics.sp?`<span class="chip">SP ${esc(mechanics.sp)}</span>`:''}<span class="chip">Seats ${esc(mechanics.seats??'—')}</span><span class="chip">${esc(mechanics.combat_speed||'—')}</span>${mechanics.nomad_access?`<span class="chip">Nomad Access ${mechanics.nomad_access}</span>`:''}</div></div>${mine?`<button class="btn-sm" data-manage-vehicle="${vehicle.instance_id}">${T('Manage Vehicle Upgrades','Управление апгрейдами транспорта')}</button>`:''}</div>${installed.length?`<div class="chips mt">${installed.map(mod=>`<span class="chip">⚙ ${esc(mod.configuration?.upgrade_name||'Upgrade')}</span>`).join('')}</div>`:`<div class="small muted mt">${T('No installed vehicle upgrades.','Нет установленных улучшений транспорта.')}</div>`}</article>`;}).join(''):`<div class="empty small">${T('No vehicles in this Dossier.','В этом Dossier нет транспорта.')}</div>`}
      </div>
    </div>
    <div>
