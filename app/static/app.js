...
function cyberdeckSheetHtml(characterData,derived,mine){
  ...
  return decks.map(deck=>{const effective=derived.effective_cyberdecks?.[deck.instance_id]||{},pools=effective.pools||{};return `<article class="panel mb cyberdeck-card"><div class="row" style="justify-content:space-between"><div><h3>${esc(deck.custom_name||deck.name)}</h3><div class="mechanic-chips">${Object.entries(pools).map(([name,pool])=>`<span class="chip">${esc(name)} ${pool.used}/${pool.total}</span>`).join('')}<span class="chip">Total ${effective.slots_used||0}/${effective.slots_total||0}</span></div></div>${mine?`<button class="btn-sm" data-manage-cyberdeck="${deck.instance_id}">${T('Manage Loadout','Управление загрузкой')}</button>`:''}</div><div class="grid cols-2 mt"><div><b>Hardware</b>${(effective.hardware||[]).map(item=>{const saved=item.backup_state?.saved_programs?.length||0;return `<div class="program-runtime-row"><span>▣ ${esc(item.name)} · ${item.slots} slots${saved?` · Saved ${saved}`:''}</span>${mine&&saved?`<button class="btn-sm" data-backup-restore="${deck.instance_id}|${item.instance_id}">${T('Restore Backup','Восстановить Backup')}</button>`:''}</div>`;}).join('')||'<div class="small muted">—</div>'}</div><div><b>Programs</b>${(effective.programs||[]).map(item=>{const runtime=item.runtime||{},status=runtime.status||'inactive',rez=runtime.rez_max?` · REZ ${runtime.rez_current}/${runtime.rez_max}`:'',attacker=runtime.category==='attacker',blackIce=runtime.category==='black_ice';return `<div class="program-runtime-row"><div><b>◆ ${esc(item.name)}</b><span class="small muted">${esc(item.program_class||'')} · ${item.slots} slots${rez}</span></div><span class="tag">${esc(status.toUpperCase())}</span>${blackIce?`<span class="small warn-text">${T('BLACK ICE ENTITY REQUIRED','ТРЕБУЕТСЯ BLACK ICE ENTITY')}</span>`:''}${mine?`<div class="row">${attacker?`<button class="btn-sm" data-program-action="${deck.instance_id}|${item.instance_id}|run">${T('Run · Manual Effect','Запустить · эффект вручную')}</button>`:''}${!attacker&&!blackIce&&status==='inactive'?`<button class="btn-sm" data-program-action="${deck.instance_id}|${item.instance_id}|rez">Rez</button>`:''}${status==='rezzed'?`<button class="btn-sm" data-program-action="${deck.instance_id}|${item.instance_id}|damage">REZ −</button><button class="btn-sm" data-program-action="${deck.instance_id}|${item.instance_id}|derez">Derez</button><button class="btn-sm" data-program-action="${deck.instance_id}|${item.instance_id}|deactivate">Deactivate</button>`:''}${status==='derezzed'?`<button class="btn-sm" data-program-action="${deck.instance_id}|${item.instance_id}|deactivate">Deactivate</button>`:''}<button class="btn-sm btn-danger" data-program-action="${deck.instance_id}|${item.instance_id}|destroy">${T('Destroy Copy','Уничтожить копию')}</button></div>`:''}</div>`;}).join('')||'<div class="small muted">—</div>'}</div></div><div class="small warn-text mt">${T('PROGRAM EFFECTS ARE MANUAL · runtime state is authoritative; Black ICE deployment arrives in B.8.3','ЭФФЕКТЫ PROGRAM ВРУЧНУЮ · runtime state серверный; развёртывание Black ICE появится в B.8.3')}</div></article>`;}).join('');
}
