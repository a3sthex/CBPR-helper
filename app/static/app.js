...
function vehicleGarageActionsHtml(vehicleEffective,characterData,derived,mine){
  const nosTanks=vehicleEffective.nos_tanks||[],mountedWeapons=vehicleEffective.mounted_weapons||[];
  if(!nosTanks.length&&!mountedWeapons.length)return '';
  ...
  const weaponHtml=mountedWeapons.map(profile=>{const resource=profile.state||{},meta=stateMetaSkill(profile.skill),stat=meta&&meta[2],statValue=stat==='EMP'?(derived.emp_cur??characterData.stats?.EMP):characterData.stats?.[stat],base=derived.effects?.skills?.[profile.skill]?.effective_check_base??((num(statValue)||0)+(num(characterData.skills?.[profile.skill])||0)),current=Number(resource.magazine??0),maximum=Number(resource.magazine_max??profile.magazine??0),reserve=Number(resource.reserve??0),ammoCost=Number(resource.ammo_cost??profile.ammo_cost??1),label=APP_I18N.current()==='ru'?profile.label_ru:profile.label_en,damage=String(profile.damage||'').match(/(\d+)d(\d+)/),mode=profile.kind==='autofire'?`Autofire ×${profile.autofire_multiplier}${profile.suppressive_fire?' · Suppressive Fire':''}`:`${profile.damage} · ROF ${profile.rof}`;return `<div class="alternate-weapon-profile vehicle-action-profile"><div><b>${esc(label)}</b><span>${esc(profile.skill)} BASE ${base} · ${esc(profile.weapon_type)} · ${esc(mode)}</span><span>${T('Facing','Направление')}: ${esc(profile.orientation||resource.orientation||'—')} · ${T('Operator','Оператор')}: ${esc(profile.operator||'driver')} · Range ${esc(profile.range_table||'—')}</span></div><div class="weapon-controls"><b>Mag ${current}/${maximum}</b><span>Reserve ${reserve}</span>${mine?`<button data-vehicle-mod-action="${profile.modification_id}|fire" ${current<ammoCost?'disabled':''}>${T('Fire','Огонь')} ×${ammoCost}</button><button data-vehicle-mod-action="${profile.modification_id}|reload" ${current>=maximum||reserve<=0?'disabled':''}>${T('Reload','Перезарядить')}</button><button data-attack-roll="${esc(profile.label_en)}|${base}">${profile.kind==='autofire'?'Autofire':'Attack'} 🎲</button>`:''}${damage?`<button data-damage-roll="${esc(profile.label_en)}|${damage[1]}|${damage[2]}|1">Damage 🎲</button>`:''}</div></div>`;}).join('');
  return `<div class="vehicle-actions mt"><h4>${T('Vehicle Actions & Mounted Weapons','Действия транспорта и бортовое оружие')}</h4>${nosHtml}${weaponHtml}</div>`;
}
...
async function viewSheet(id) {
  ...
  $$('[data-vehicle-mod-action]',view).forEach(button=>button.onclick=async()=>{const [modificationId,action]=button.dataset.vehicleModAction.split('|'),body={revision:c.revision,action};if(action==='reset_nos'){const reason=prompt(T('Campaign day reset reason','Причина сброса игрового дня'),T('New campaign day','Новый игровой день'))||'';if(reason.trim().length<3)return;body.reason=reason;}if(action==='use_nos'&&!confirm(T('Spend this NOS tank for the current campaign day?','Потратить этот баллон NOS в текущем игровом дне?')))return;button.disabled=true;try{await api(`/api/characters/${c.id}/modifications/${modificationId}/action`,{method:'POST',body});toast(action==='use_nos'?T('NOS spent. Resolve the additional Move Action manually.','NOS потрачен. Примените дополнительное Move Action вручную.'):T('Vehicle resource updated.','Ресурс транспорта обновлён.'));viewSheet(c.id);}catch(error){button.disabled=false;toast(error.message,true);}});
