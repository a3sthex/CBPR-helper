...
def build_base(wb):
    ...
    put(ws, 'G4', '=IF(D36<0,"⚠ ПОРОГ ПРОЙДЕН",IF(D36<10,"⚠ НА ГРАНИ","—"))', kind='auto')
    ...
    rows = [
        ...
        (38, 'СТАТУС HUMANITY', '=IF(D36<0,"⚠ КИБЕРПСИХОЗ",IF(D36<10,"⚠ НА ГРАНИ",'
    ...
    memo = [
        ...
        'Hardened / SP: урон меньше SP = 0 урона, но броня аблейтится на 1 (при пробитии — тоже на 1).',
    ...
    cf_warn(ws, 'D36', 'OR($D$36<10,$D$36<0)')
...
def build_chrome(wb):
    ...
    cf_warn(ws, 'D37', 'OR($D$37<10,$D$37<0)')
    cf_warn(ws, 'D36', 'OR($D$36<10,$D$36<0)')
    return ws
...
def build_gear(wb):
    ...
    put(ws, 'D5', f'=$I$3+$I$4+SUM($E${C_FIRST}:$E${C_LAST})', kind='auto', fmt=CASH_FMT)
    ...
    mput(ws, 'I5:M5', 'Неиспользованные стартовые евродоллары после создания сгорают. +1500 €$ «от корпорации» — '
                      'только на импланты, и вместе с обязательным Neural Link.', kind='note')
    ...
    for r in range(C_FIRST, C_LAST + 1):
        ...
        put(ws, f'F{r}', f'=$I$3+$I$4+SUM($E${C_FIRST}:$E{r})', kind='auto', fmt=CASH_FMT)
    ...
    put(ws, f'F{tr}', f'=$I$3+$I$4+SUM($E${C_FIRST}:$E${C_LAST})', kind='auto', fmt=CASH_FMT)
    ...
    mput(ws, f'B{rr}:M{rr}', 'ПАМЯТКА ПО ДЕНЬГАМ: 2550 €$ — оружие/броня/снаряжение/хром; 800 €$ — только Fashion + Fashionware; '
                             'Kibble Lifestyle (100 €$/мес) и жильё покрыты на старте; +1500 €$ «от корпорации» — только на импланты '
                             'и вместе с обязательным Neural Link; неиспользованное после создания сгорает. В 2070-х можно купить почти всё '
...
def build_reference(wb, data):
    ...
    r = 3
    mput(ws, f'B{r}:N{r}', 'Ⅰ · СЛОЖНОСТЬ ПРОВЕРОК (DV)  ·  CP:R СТР. 129', kind='h1')
    r += 1
    ...
    for label, mod, text in [
        ...
        ('Картечь (Shells)', 'DV13', 'Все видимые цели в квадрате 6×6 м получают 3d6 (один бросок урона на всех).'),
    ...
    r += 2
    mput(ws, f'B{r}:N{r}', 'Ⅶ · IP: СКОЛЬКО СТОИТ РАЗВИТИЕ  ·  CP:R СТР. 408', kind='h1')
    r += 1
    ...
    for label, price, note in [
        ...
        ('Trauma Team Silver', '500 / год', 'Подписка: привозят в больницу и стабилизируют.'),
        ('Trauma Team Executive', '1 000 / год', 'Плюс лечение и VIP-палата.'),
    ...
    for q, a in [
        ...
        ('Что делать, если урон меньше SP?', 'Урон не проходит, но броня всё равно аблейтится на 1 (если урон её не пробил). При пробитии — тоже на 1.'),
        ...
        ('Как лечится серьёзное ранение?', 'Естественно — 1 HP в день отдыха при стабилизации; Stim снимает штраф на час, Speedheal лечит BODY+WILL HP.'),
    ]:
        r += 1
    ...
    blocks = [
        ...
        ('Ⅱ · ШАГИ СОЗДАНИЯ ПЕРСОНАЖА (ГАЙД SPES DESPERATA)', [
            ...
            ('6. Закупка', '2550 €$ на оружие, броню, снаряжение и хром + 800 €$ на Fashion и Fashionware. Бюджет считается сам: '
                           'инвентарь + броня + установленный хром; Neuroport при создании бесплатен (0 €$, 0 HL). '
                           'Неиспользованное сгорает.'),
            ('7. Lifestyle', 'Kibble (100 €$/мес) и жильё покрыты. Дальше платишь сам — см. СПРАВКУ.'),
        ]),
