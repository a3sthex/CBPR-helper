...
**Дата:** 2026-08-21 (актуализирован 2026-08-28, раздел §24)  
...
1. ◐ Разделить `server.py`, `app.js`, `ncnet.js`, CSS. **Backend — ✅ 2026-08-28** (`server.py` 20 217 → 805 строк, 21 модуль app/, 260/260 тестов на каждом шаге; см. §24.1 и `docs/architecture.md`). **Frontend — ◐ 2026-08-29, срезы S1–S4:** S1 `viewCalc` → `views-quickref.js` (211); S2 кластер досье → `views-dossiers.js` (644); S3 редактор персонажа → `views-editor.js` (472); S4 мастер создания + общие render-хелперы → `views-shared.js` (1 702). **`app.js` 4 389 → 1 369 строк (−69 %)**. Принятая механика срезов, порядок загрузки и тестовые контракты — §24.5. Осталось: разделение `views-shared.js` (срез S5), чёрный рынок / мастер-панель / ростер, CSS.
...
0. ◐ **(20.4)** Перенести все функции Map на страницу в **City** (вместо отдельного раздела). **Срез 20.4.1 — ✅ 2026-08-29:** карта вошла в раздел City (вход «ПОЛНАЯ КАРТА →» из консоли GEOSPATIAL RELAY на странице City, хлебная крошка `NC//NET CITY // GEOSPATIAL RELAY` и «← Город» на карте, маршрут `#/map` подсвечает City в навигации, отдельного пункта навигации у карты нет); GM/Admin ставят custom POI **кликом по карте** (`ncBindMapPlacement` → `openLocationEditor(null,{x,y})`, координаты считаются через `getScreenCTM()`, курсор crosshair). **Осталось:** zoom/pan и слои довести до фильтров на City-странице, marker clustering, расстановка custom POI прямо из консоли City (сейчас — с полной карты).
...
7. Печатный лист должен быть на 1, 2 или 3 страницах?  → **Решено:** печатный лист — по образцу официального fillable PDF из репозитория (`library/RTG-CPR-CharacterSheet-Fillable.pdf`), landscape, ~2 страницы.
...
- **23.1** Упрощение модалей (Feed/Contracts/Storyline) + **схлопывание City Feed в один тип поста**. **Сделано 28.08 ✅:** `FEED_FORMATS = {'post'}` + `FEED_DEFAULT_FORMAT='post'`; миграция 22 (`city feed single post type`) схлопывает исторические строки; API склейвает старые форматы в `post` вместо 400; UI-композер/лейблы унифицированы; регрессионный тест `test_legacy_feed_formats_collapse_to_single_post_type` (261/261 зелёные).
...
- ◐ **23.3** Демонтаж Session + «панель действий игрока». Панель действий — **реализована** (`#/actions/<char_id>` + кнопка `🎮 Actions` в Dossier). **Срез от 28.08:** GM-дашборд сессии разбит на вкладки ⚔ Бой / ◈ NET / 🎛 Операции (Access, Safety-onboarding, заметки, журнал, export, «⇄ В досье» — в операциях). **Осталось:** вынос боевого трекера в VTT-направление и доводка «панели действий» как единого поля боя игрока.
...
Полный прогон: `python3 -m unittest discover -s tests` (252 теста; **актуально на 28.08 — 260 тестов**), `node --check` по всем JS, ручной security-review `server.py` и фронтенда.
...
**Исправлено 2026-08-28:** нормализация экзотической нотации выполнена на обоих уровнях — в `import_data.py` (при загрузке Data Pool) и в runtime-нормализаторе `catalog.py`; поле приходит числом, date-dependent кейс больше не хрупок (§24.2).
...
- ✅ **§23.1:** один тип поста в City Feed — закрыто 28.08 (миграция 22, мягкая склейка, тест).
...
## 24. Актуализация и дополнения (2026-08-28)

### 24.1 Выполнено сегодня: backend-декомпозиция server.py (P3 #1, backend-часть)
...
### 24.2 Статусные исправления документа (сверка с кодом 28.08)

| Пункт | Было в документе | Факт 28.08 |
|---|---|---|
...
| §17 P3 #1 | не начато | backend ✅; frontend ◐ (срезы S1–S4, 2026-08-29: app.js −69 %) |
...
- ✅ **23.1 схлопывание City Feed в один тип** — сделано 28.08: один «пост», миграция 22 + мягкая склейка на API-слое; модал-упрощение из той же строки §21 остаётся кандидатом на отдельный проход вместе с P3-frontend.
- **20.4 Map → City** — ◐ срез 20.4.1 сделан 29.08: карта — часть City (вход из City-консоли, крошка и подсветка City), GM ставит POI кликом. Отдельного пункта навигации у карты нет уже в коде; статусная пометка про «отдельный 🗺️ Map» устарела. Остаток — zoom/pan/слои/clustering на странице City.
- **23.3 «панель действий игрока»** — ◐ наполовину: `player-actions.js` загружен на странице, но маршрута/входа из Dossier в `app.js` не найдено → прогнать глазами в UI, при отсутствии входа — доделать маршрут + кнопку.
- **H.3 one-click боевые действия** — после решения 23.3 НЕ строить в существующем Session Dashboard (он под демонтаж); целится в панель действий игрока / VTT. Каноническая привязка обновлена в Пакете H #3.
- **A.2 №№4–5 (reputation gates, multi-vendor offers)** — остаются заблокированными за P5 (Организации): не трогать до memberships + Reputation/Favor/Heat.
- **Изображения из книг → блокер инфраструктурный:** бинарники (~277 MB, 11 файлов) живут только на Яндекс.Диске; `library/` содержит указатели по ~132 байта. Из песочницы достать нельзя. Варианты: (а) загрузить файлы в чат/репо; (б) локальный скрипт извлечения (embedded-растры + рендеры страниц через PyMuPDF) — готовится по команде.

### 24.4 Перекалибровка ближайших направлений (рекомендация, не замена §21)

1. **P-UX 23.3 — Session-демонтаж:** старт с доводки player-actions-страницы (маршрут + кнопка из Dossier), затем демонтаж combat-UI Session; бэкенд-границу уже зафиксировала декомпозиция (`sessions_api.py`).
2. **P3 frontend:** `app.js` по доменам вью (feed/dossiers/market/sessions/…) тем же миксин-конвейером; выигрыш: любые дальнейшие UX-работы (23.1, 20.7, 20.4, P-UX) не ковыряют монолитный JS.
...
- **20.4.1 (✅) отдельно от P3:** backend к этому моменту уже умел create/update/delete POI с координатами и разрешал GM архивировать каноничные seed-POI 2077 (тест `test_seed_location_is_read_only_and_player_cannot_write`), поэтому срез коснулся только фронтенда; добавлен контрактный тест `test_city_page_owns_the_map_and_gm_places_pois_by_click`.
- **Срез S4 (✅):** мастер создания персонажа (7 шагов) + общие render-хелперы → `app/static/views-shared.js` (1 702 строки, 14 top-level констант, ~144 функции). Блок резался **целиком**, без разбора по функциям: внутри перемешаны wizard-функции и хелперы, которыми пользуются `views-dossiers.js` и `views-editor.js` (`groupedItemsHtml`, `itemMechanicChips`, `showSkillInfo`, `skillCategoryLabel`, `cyberwareLifecycleHtml` и др.). Разделение на `views-shared.js` + `views-wizard.js` — срез S5.
- **Зависимость среза S4:** `SUB_SKILL_BASES` инициализируется из `SPECIALIZED_SKILL_BASES`, которая живёт в `creation-data.js`, — поэтому `views-shared.js` стоит в `index.html` **после** `creation-data.js`. Попытка разбирать блок по функциям скриптом провалилась (шаблонные строки ломают границы `}` на нулевой колонке) — простой механический перенос секции надёжнее.
...
- **Следующие кандидаты на срезы S5+:** разделить `views-shared.js` на общие хелперы и мастер создания (нужен аккуратный парсер по функциям или ручная разметка), чёрный рынок (302 строки в `app.js`), мастер-панель `viewAdmin`, ростер / новости / доска заказов / вход-профиль, затем CSS (`style.css` 765, `ncnet.css`). После S4 `app.js` — 1 369 строк, дальше деление даёт меньше выгоды, чем возврат к фичам (23.3, 20.4).
