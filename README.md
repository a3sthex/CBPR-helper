...
`tools/build_character_sheet_xlsx.py` собирает из `app/data/items.json` автономный `.xlsx` для игры за столом — результат `NC-NET_CharSheet_2070.xlsx` импортируется в Google Таблицы (макросов нет, все формулы совместимы). Листы: **01 ДОСЬЕ** (дело NCPD с фото и сводкой), **02 ОСНОВА** (62 очка статов, HP, Humanity, Combat Awareness Solos), **03 НАВЫКИ** (66 навыков + проверка 86 очков), **04 ХРОМ** (HL, срез максимума Humanity, 2070-й Neuroport), **05 СНАРЯЖЕНИЕ** (бюджеты 2550/800 €$, оружие, броня, инвентарь, касса), **06 СОСТОЯНИЕ** (крит-травмы, зависимости, IP, лог дел), **КАТАЛОГ** на 1092 позиции, **СПРАВКА** и **ПРАВИЛА**. Штрафы брони, SP, базы атак и бюджеты считаются автоматически по правилам проекта (лучший SP в локации, самый строгий штраф применяется один раз).

**Дизайн.** Лист **01 ДОСЬЕ** — красно-чёрное дело NCPD (бумага, штампы, фото объекта), а **02–06** — единый тёмный протокол: углеродный фон `#14141A`, авто-ячейки `#0F0F15`, красные акценты `#FF2A3C`, янтарная подсветка полей ввода `#241A10`. Палитры живут в генераторе (`PAL_NCPD`, `PAL_DARK`) — тему листа переключает `use_theme('ncpd'|'dark')`. Превью всех листов без открытия файла: `sheets/preview-01-dose.png` … `preview-06-sostoyanie.png` — рендерятся `tools/render_sheet_preview.py` (нужны Pillow и formulas).

```bash
python3 -m pip install --target .build-deps openpyxl pillow
PYTHONPATH=.build-deps python3 tools/build_character_sheet_xlsx.py   # → NC-NET_CharSheet_2070.xlsx
```

### Импорт в Google Таблицы и игра с листом

1. Google Диск → **Создать → Загрузка файла** → выбрать `NC-NET_CharSheet_2070.xlsx`.
2. Правый клик по файлу → **Открыть с помощью → Google Таблицы** (либо в пустой таблице `Файл → Импорт → Загрузить`).
3. После импорта проверить три вещи: выпадающие списки на 04 и 05 (значения тянутся из листа `КАТАЛОГ`), проверку очков на 03 («✔ 86 очков») и проверку бюджетов на 05 («✔ в рамках стартовых бюджетов»).
4. Служебные листы `КАТАЛОГ`, `СПРАВКА`, `ПРАВИЛА` → правый клик по вкладке → **Защитить лист** (на них ссылаются формулы).
5. Фото в досье: правый клик по заглушке → **Заменить изображение**; имя, позывной, возраст и игрок — жёлтые поля в 01.

Правила работы с листом: пишем только в подсвеченные поля ввода (жёлтые на 01, янтарные на 02–06); строки и столбцы внутри блоков не удалять и не вставлять — формулы считают фиксированные диапазоны, а свободные строки уже заложены (оружие 3–8, броня 4–6, инвентарь № 20, зависимости/цели/табу на 06, лог дел 06). В игре: текущие HP — 02, патроны и добро — 05, крит-травмы, зависимости, IP и лог дел — 06.
