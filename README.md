...
| Лист | Что внутри |
| --- | --- |
| **01 ДОСЬЕ** | Дело объекта в две колонки: личность и мотивация, служба (Lifepath Lawman), прошлое, связи, служебная сводка с игровых листов. Фото — в правом верхнем углу. |
| **02 ОСНОВА** | Характеристики (62 очка), производные (HP, Humanity, инициатива, MOVE), Backup (Lawman), оружие в руках, броня и памятка бойца. |
| **03 НАВЫКИ** | 66 навыков + 6 свободных строк; пишешь только уровень. Справа — панель проверок: 86 очков, максимум 6, 13 обязательных и пакет роли. |
| **04 ХРОМ** | Импланты: HL по среднему, срез максимума Humanity, клиника, слоты, терапия и таблица «что режет Humanity» (хром, события, жизнь). |
| **05 СНАРЯЖЕНИЕ** | Бюджеты 2550 €$ и 800 €$ на стиль, оружие, броня, инвентарь и касса кампании. |
| **06 СОСТОЯНИЕ** | Трекер крит-травм (локация + 2d6 → травма подтягивается из СПРАВКИ), зависимости, IP, лог дел. |
| **КАТАЛОГ / СПРАВКА / ПРАВИЛА** | 1092 позиции из Data Pool, справочник DV/укрытий/крит-травм/IP и инструкция. |

Штрафы брони, SP, базы атак и бюджеты считаются автоматически (лучший SP в локации, самый строгий штраф применяется один раз). Превью всех листов без открытия файла: `sheets/preview-01-dose.png` … `preview-06-sostoyanie.png` — рендерятся `tools/render_sheet_preview.py` (нужны Pillow и formulas).

### Импорт в Google Таблицы и игра с листом

После импорта проверить три вещи: выпадающие списки на 04 и 05 (значения тянутся из листа `КАТАЛОГ`), проверку очков на 03 («✔ 86 очков») и проверку бюджетов на 05 («✔ в рамках стартовых бюджетов»).
4. Служебные листы `КАТАЛОГ`, `СПРАВКА`, `ПРАВИЛА` → правый клик по вкладке → **Защитить лист** (на них ссылаются формулы).
5. Фото в досье: правый клик по заглушке → **Заменить изображение**; имя, позывной, возраст и игрок — янтарные поля в 01.

Правила работы с листом: пишем только в янтарные поля ввода; строки и столбцы внутри блоков не удалять и не вставлять — формулы считают фиксированные диапазоны, а свободные строки уже заложены (оружие 3–8, броня 4–6, инвентарь № 20, зависимости/цели/табу на 06, лог дел 06). В игре: текущие HP — 02, патроны и добро — 05, крит-травмы, зависимости, IP и лог дел — 06.
