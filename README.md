...
| Раздел | Описание |
| --- | --- |
| 🧬 **Мои персонажи** | Мастер с persistent draft, каруселью 10 ролей, объединённым CP:R+CEMK Lifepath и региональными генераторами имён; строгая проверка 62/86, parent-pool специализаций, 66 навыков с STAT/LVL/BASE, корзины Fashion/снаряжения/хрома, раздельная Humanity и броня головы/тела. У каждого пользователя — своё хранилище. |
...
- Draft мастера версионирован и автоматически хранится в `localStorage` отдельно для каждого пользователя до успешного создания или подтверждённого полного сброса. Старые персонажи и старые схемы Lifepath остаются читаемыми.
