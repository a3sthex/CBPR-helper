...
## Лист персонажа в Google Таблицах (офлайн-шаблон)

`tools/build_character_sheet_xlsx.py` собирает из `app/data/items.json` автономный `.xlsx` для игры за столом — результат `NC-NET_CharSheet_2070.xlsx` импортируется в Google Таблицы (макросов нет, все формулы совместимы).

**Дизайн — HUD Night City:** чёрный фон, красный `#FF003C`, белый текст и оранжево-жёлтый `#FFB100` для полей ввода и заголовков — как интерфейс в Cyberpunk 2077. Шрифты: **Oswald** (крупные заголовки), **Roboto** (текст), **Roboto Mono** (цифры и метки); все три есть в Google Таблицах. Все листы альбомной ориентации и печатаются по ширине страницы.

| Лист | Что внутри |
| --- | --- |
| **01 ДОСЬЕ** | Досье от фиксера (Fixer Database): ID — позывной, имя, роль/чем живёт, ранг, игрок, возраст, языки, корни, район, стиль одежды, рост/вес, хром на виду, приметы, голос, связь, жильё. I психопортрет (личность, что ценит, отношение к людям, мораль, цель, чего никогда не сделает, слабость/крючок). II биография General Lifepath (семья, кризис, друзья, романтика, враги, переломный момент). III как работает — оценка фиксера, универсально для любой роли (кем был/где работал/честность отдела/кто давил/над чем работали + за что ушёл/что скрывает/связи/подписки/кто охотится/репутация/фиксер/правила работы/что берёт-не берёт/цена/надёжность). IV связи (6), V приводы/дела (5), VI оценка фиксера (уровень угрозы, вооружение, хром, психо, принадлежность, заметка фиксера). VII автосводка с 02/03 (HP, SP, оружие, наличные, крит, IP, Backup). VIII заметки фиксера/GM, IX границы безопасности (Lines/Veils). Фото F4:G12. |
| **02 ИГРА** | Всё для стола на одном листе: I характеристики (62 очка, 2–8, штраф брони тянется с 03), II производные (HP, порог серьёзно ранен, спасбросок смерти, инициатива, MOVE, Humanity база/срез/макс/HL/текущая/EMP/статус), III быстрый бой (SP головы/тела/щит, штраф REF/DEX/MOVE, уклонение, состояние ран, LUCK, наличные, Humanity, крит-активных), IV оружие в руках (4 слота авто из 03 — урон/ROF/магазин/патроны/навык/база атаки/заметки/эффект), V навыки (66+6 свободных, пишешь только УР., база = УР.+стата, ×2 стоит 2 очка) + справа панель проверок: раскидано/остаток/макс 6, 13 обязательных (мин.2), пакет роли Lawman (★) и универсальный Solo-пакет (☆), быстрый хром (топ 6 установленных) и крит-активные (3) + памятка бойца (6 пунктов). |
| **03 КАМПАНИЯ** | Учёт между сессиями: бюджеты 2550 €$ (оружие/броня/снаряжение/хром) + 800 €$ (Fashion/Fashionware) с авто-проверкой, наличные авто. I хром детально (14 слотов — название из КАТАЛОГа → категория/подтип/цена/HL/срез/установлен/клиника/эффект/в бюджет/в стиль + описание), итоги HL/срез/бюджеты, Neuroport проверка (2070: любой немедицинский хром требует Neuroport, при создании 0 €$/0 HL). II Humanity дубль с 02 + шкала █/░. III терапия (Standard/Extreme/Addiction). IV оружие мастер (8 слотов, навык/урон/ROF/магазин/патроны/качество/в руках 1–4/база атаки/заметки/цена/в бюджет/эффект). V броня и щит (6 слотов — локация голова/тело/щит, надето, SP/HP/штрафы/SP сейчас/эфф.SP/заметки/цена/в бюджет) + авто SP головы/тела/щит HP и штраф брони. VI инвентарь (20 слотов — категория/подтип/кол-во/цена/цена своя/сумма/где носит/заметка/описание) + итог. VII касса (20 операций дата/что/оборот/баланс/заметка). VIII крит-травмы (10 — локация тело/голова + 2d6 → авто травма/эффект/быстрый фикс/лечение/статус из СПРАВКИ). IX зависимости/репутация/IP (заработано/потрачено авто/ свободно). X что улучшили за IP (10 строк — тип НАВЫК/×2/РОЛЬ/СТАТА, новый УР., стоимость авто 20/40/60×). XI лог дел (14 сессий). XII долги/цели/табу (свободные). |
| **КАТАЛОГ / СПРАВКА / ПРАВИЛА** | 1092 позиции из Data Pool (скрытый лист, на него ссылаются формулы — автоподстановка цены/урона/SP/HL/штрафов/описания), справочник DV/дистанций/автоогня/прицельных/укрытий/действий/ран/IP/жилья/2070-твиков/Backup рангов + крит-травмы тела/головы, и инструкция по листу. Служебные листы скрыты. |

Штрафы брони, SP, базы атак и бюджеты считаются автоматически (лучший SP в локации, самый строгий штраф применяется один раз). Превью всех листов без открытия файла: `sheets/preview-01-dossier.png` (заполненный пример Dose/Lawman), `preview-01-dossier-empty.png` (пустой универсальный), `preview-02-play.png`, `preview-03-campaign.png` — рендерятся `tools/render_sheet_preview.py` (нужны Pillow и formulas).

### Импорт в Google Таблицы и игра с листом

1. Импортируй `sheets/NC-NET_CharSheet_2070.xlsx` (пустой универсальный) — File → Import → Replace. `NC-NET_CharSheet_2070_EXAMPLE_Lawman.xlsx` (пример законника в отставке Dose) — для сверки.
2. После импорта проверить три вещи: выпадающие списки в 03 КАМПАНИЯ (оружие/броня/хром/инвентарь тянутся из скрытого `КАТАЛОГ`), проверку очков на 02 ИГРА («✔ 86 очков», «✔ все 13 закрыты», «✔ 62 очка») и проверку бюджетов на 03 КАМПАНИЯ («✔ в рамках стартовых бюджетов»).
3. Шрифты Oswald/Roboto/Roboto Mono — есть в Google Таблицах. Все листы альбомные, печать по ширине (File → Print → Fit to width).
4. Служебные листы `КАТАЛОГ`, `СПРАВКА`, `ПРАВИЛА` → правый клик по вкладке → **Защитить лист** (на них ссылаются формулы, сами листы скрыты — Format → Show hidden sheets чтобы увидеть).
5. Фото в досье: правый клик по заглушке F4:G12 → **Заменить изображение** (рекомендуемый размер 460×250 ~1.84:1); имя, позывной, возраст и игрок — янтарные поля в 01.

Правила работы с листом: пишем только в янтарные поля ввода; строки и столбцы внутри блоков не удалять и не вставлять — формулы считают фиксированные диапазоны (статы B6:B15, навыки C36:C107, хром C9:C22, оружие C48:C55, броня C60:C65, инвентарь C72:C91, касса E96:E115, крит C120:C129, IP C141:C150, лог C155:C168). В игре: текущие HP/штраф спасбросков/LUCK/патроны — 02 ИГРА (верх и оружие), детальный учёт хрома/брони/инвентаря/кассы/крит-травм/IP/лога — 03 КАМПАНИЯ, а 01 ДОСЬЕ — для отыгрыша и GM (связи, инциденты, оценка угрозы, сводка авто).
